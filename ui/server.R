...
player = readRDS("./../data/player_active.rds") %>% filter(status=="Active") %>% filter(position %in% c("RB","QB","WR"))
play_player = readRDS("./../data/play_player_active.rds") %>% filter(player_id %in% player$player_id)
